...
from mag_nODE_model.train.training_clean import _training_subset_time as _training_
...
for cv in range(0, _cvs):
    ...
    tr_loss, vd_loss, r2, niter = _training_(niters, func, device, dl_list, vdl_list, batch_t_list, lr = float(config['lr']), wd = float(config['wd']), decay = config['decay'])
    final_niter = niter
